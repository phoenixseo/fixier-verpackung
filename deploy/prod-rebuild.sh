...
# ##################################################################
# Locations:
# project/deploy/prod-rebuild.sh = location of this file
...
# Check absolute path
# This Script sits in project/deploy/
cd ..
PROJECT_PATH=$(pwd)

DRUPAL_ROOT=$PROJECT_PATH/$DRUPAL_DIR

# simple prompt Routine #############################################
prompt_yes_no() {
  while true ; do
printf "$* [Y/n] "
    read answer
...
if [ ! -d $PROJECT_PATH/deploy/$LOG_DIR ]; then
    mkdir $PROJECT_PATH/deploy/$LOG_DIR
    chmod 700 $PROJECT_PATH/deploy/$LOG_DIR
fi

logfile=$PROJECT_PATH/deploy/$LOG_DIR/prod-rebuild-$now.log
...
if prompt_yes_no "Do you want to PULL THE GIT REPOSITORY" ; then
  ...
  # make certain files overwriteable before git pull
  chmod 644 $DRUPAL_ROOT/$SITE_DIR/settings.php 2>&1 | tee -a $logfile
  chmod 644 $DRUPAL_ROOT/$SITE_DIR/services.yml 2>&1 | tee -a $logfile
  chmod 644 $DRUPAL_ROOT/$SITE_DIR/settings/settings.local.php 2>&1 | tee -a $logfile
  chmod 644 $DRUPAL_ROOT/$SITE_DIR/settings/settings.prod.php 2>&1 | tee -a $logfile
  chmod 644 $DRUPAL_ROOT/$SITE_DIR/settings/settings.shared.php 2>&1 | tee -a $logfile
